...
private: int size;//размер вектора 
...
{ 
	if (index < size) return data[index-1];
	else cout << "\nError! Index>size"; 
} 

//операция для добавления константы 
template <class T> Vector<T> Vector<T>::operator+(const T k)//+k
{ 
